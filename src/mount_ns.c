...
int copy_image(const char *img_path, int init_pid, char *buff)
{
    ...
    snprintf(tar_command, sizeof tar_command, "tar -czf /tmp/aucont_image_%d.tar.gz -C %s ../$(basename %s)", init_pid, img_path, img_path);
    if (system(tar_command) != 0) {
        pr_err("%s", "Could not copy image dir\n");
        goto err;
    }

    snprintf(untar_command, sizeof untar_command, "tar -xzf /tmp/aucont_image_%d.tar.gz -C %s", init_pid, tmp_dir);
