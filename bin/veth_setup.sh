...
ip link add veth0_$1 type veth peer name veth1                         && echo "veth pair created (veth1 in container and veth0_$1 in host)"
...
ip netns exec $netns_name ip address add $container_ip/24 dev veth1    && echo "veth1 ip address set to $container_ip"
ip link set veth0_$1 up && ip address add $host_ip/24 dev veth0_$1     && echo "veth0 up and ip address set to $host_ip"
ip netns exec $netns_name ip route add default via $host_ip            && echo "default route throught veth0"
...
echo "nameserver 8.8.8.8" > /etc/resolv.conf                           && echo "8.8.8.8 DNS server added in global resolv.conf"
